...
<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
    <div class="flex-1">
        <div class="flex items-center gap-3 mb-2">
            ...
            <?php elseif ($daysUntilDue <= 7 && $daysUntilDue > 0 && $invoice['status'] !== 'paid'): ?>
        ...
        <div class="text-gray-600 space-y-1">
            ...
            <p>Created: <?php echo Helper::formatDate($invoice['invoice_date'], 'M j, Y'); ?> • 
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
        <?php if ($invoice['status'] !== 'paid' && $invoice['balance_amount'] > 0): ?>
        ...
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-xs text-green-600 mt-1">
                    Amount: <?php echo Helper::formatCurrency($invoice['total_amount']); ?> • 
                    Paid Amount: <?php echo Helper::formatCurrency($invoice['paid_amount']); ?>
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-xs text-red-600 mt-1">
                    Due date: <?php echo Helper::formatDate($invoice['due_date'], 'M j, Y'); ?> • 
                    Outstanding: <?php echo Helper::formatCurrency($invoice['balance_amount']); ?>
                </p>
            </div>
        </div>
    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-sm font-medium <?php echo $isOverdue ? 'text-red-600' : 'text-gray-900'; ?>">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">
                                            Payment of <?php echo Helper::formatCurrency($payment['amount']); ?>
                                        </p>
                                        <p class="text-sm text-gray-600">
                                            <?php echo Helper::formatDate($payment['payment_date'], 'M j, Y'); ?>
                                            <?php if (!empty($payment['payment_method'])): ?>
                                                • via <?php echo ucwords($payment['payment_method']); ?>
                                            <?php endif; ?>
                                        </p>
                                        <?php if (!empty($payment['notes'])): ?>
                                            <p class="text-sm text-gray-500 mt-1"><?php echo htmlspecialchars($payment['notes']); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                ...
                <?php if ($invoice['total_amount'] > 0): ?>
                    <div class="pt-2 border-t border-gray-200">
                        <div class="flex justify-between text-xs text-gray-600 mb-2">
                            ...
                            <span><?php echo round(($invoice['paid_amount'] / $invoice['total_amount']) * 100, 1); ?>%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-green-500 h-2 rounded-full" 
                                 style="width: <?php echo min(100, ($invoice['paid_amount'] / $invoice['total_amount']) * 100); ?>%"></div>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
...
document.addEventListener('keydown', function(e) {
    ...
    switch(e.key.toLowerCase()) {
        ...
        case 'p':
            if (!e.ctrlKey && !e.metaKey) {
                e.preventDefault();
                window.open('<?php echo Helper::baseUrl('modules/invoices/preview_invoice.php?id=' . Helper::encryptId($invoice['id'])); ?>', '_blank');
            }
            break;
        case 'd':
            if (!e.ctrlKey && !e.metaKey) {
...
window.addEventListener('beforeprint', function() {
    document.title = 'Invoice #<?php echo htmlspecialchars($invoice['invoice_number']); ?> - <?php echo htmlspecialchars($invoice['company_name']); ?>';
});
